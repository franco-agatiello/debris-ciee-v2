...
function poblarFiltros() {
    ...
    menuPais.querySelectorAll('.dropdown-item').forEach(item=>{
    ...
    menuClase.querySelectorAll('.dropdown-item').forEach(item=>{
      item.addEventListener('click', function(e){
        e.preventDefault();
        document.getElementById('dropdownClaseBtn').textContent = this.textContent;
        document.getElementById('dropdownClaseBtn').dataset.value = this.dataset.value;
        actualizarMapa();
      });
    });
    ...
    const inclinacionMaxVal = document.getElementById("inclinacion-max-val");

    inclinacionMaxSlider.value = 180;
    inclinacionMaxVal.textContent = 180;
    
    inclinacionMinSlider.oninput = () => {
      if (Number(inclinacionMinSlider.value) > Number(inclinacionMaxSlider.value)) {
        inclinacionMaxSlider.value = inclinacionMinSlider.value;
      }
      inclinacionMinVal.textContent = inclinacionMinSlider.value;
      actualizarMapa();
    };
    inclinacionMaxSlider.oninput = () => {
      if (Number(inclinacionMaxSlider.value) < Number(inclinacionMinSlider.value)) {
        inclinacionMinSlider.value = inclinacionMaxSlider.value;
      }
      inclinacionMaxVal.textContent = inclinacionMaxSlider.value;
      actualizarMapa();
    };

    const masaMinSlider = document.getElementById("masa-min");
    const masaMaxSlider = document.getElementById("masa-max");
    const masaMinVal = document.getElementById("masa-min-val");
    const masaMaxVal = document.getElementById("masa-max-val");

    masaMaxSlider.max = maxMasa;
    masaMaxSlider.value = maxMasa;
    masaMaxVal.textContent = maxMasa;

    masaMinSlider.max = maxMasa;
    masaMinSlider.oninput = () => {
      if (Number(masaMinSlider.value) > Number(masaMaxSlider.value)) {
        masaMaxSlider.value = masaMinSlider.value;
      }
      masaMinVal.textContent = masaMinSlider.value;
      actualizarMapa();
    };
    masaMaxSlider.oninput = () => {
      if (Number(masaMaxSlider.value) < Number(masaMinSlider.value)) {
        masaMinSlider.value = masaMaxSlider.value;
      }
      masaMaxVal.textContent = masaMaxSlider.value;
      actualizarMapa();
    };
}
...
function listeners(){
    ["fecha-desde","fecha-hasta"].forEach(id=>{
