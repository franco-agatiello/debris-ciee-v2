...
function poblarClaseObjetos() {
  const clases = Array.from(new Set(debris.map(d => d.clase_objeto).filter(c => c && c !== null)));
  clases.sort((a, b) => a.localeCompare(b, 'es'));
  const menu = document.getElementById("dropdownClaseMenu");
  menu.innerHTML = `<li><a class="dropdown-item" href="#" data-value="">Todas</a></li>` +
    clases.map(c => `<li><a class="dropdown-item" href="#" data-value="${c}">${c}</a></li>`).join('');
  menu.querySelectorAll('.dropdown-item').forEach(item => {
    item.addEventListener('click', function(e){
      e.preventDefault();
...
function filtrarDatos() {
  ...
  return debris.filter(d => {
    ...
    if (filtros.masaOrbitaMin && (!d.masa_en_orbita || Number(d.masa_en_orbita) < Number(filtros.masaOrbitaMin))) return false;
    if (filtros.masaOrbitaMax && (!d.masa_en_orbita || Number(d.masa_en_orbita) > Number(filtros.masaOrbitaMax))) return false;
    if (filtros.clase_objeto && d.clase_objeto !== filtros.clase_objeto) return false;
    return true;
  });
}
...
function listeners(){
  ["fecha-desde","fecha-hasta","inclinacion-min","inclinacion-max","masa-orbita-min","masa-orbita-max"].forEach(id=>{
...
// Modals y visualización de trayectorias/orbitas igual que tu original:
// Puedes copiar tu código existente para: mostrarTrayectoria, mostrarOrbitaPlanta, mostrarOrbita3D
